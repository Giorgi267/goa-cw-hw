...
document.getElementById("submit-button").addEventListener("click", function (event) {
    ...
    if (!originalListing) {
        console.error("No template listing found!");
        return;
    }
    ...
    clone.classList.add(inputs[2].value);
    location.textContent = inputs[3].value; 
    beds.innerHTML = `${inputs[4].value} <i class="fa-solid fa-bed"></i>`;
    quickText.textContent = inputs[5].value; 
    ...
    document.querySelector(".inputs-container").reset();
});
